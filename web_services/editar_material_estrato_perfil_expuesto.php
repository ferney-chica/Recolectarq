...
$conexion=mysqli_connect($hostname,$username,$password,$database);
	$material_estrato_perfil_id = $_POST["material_estrato_perfil_id"];
	...
	//$sql="UPDATE usuarios SET nombre= ?, apellido= ? WHERE usuario_id=?";
      $sql="UPDATE `materiales_estratos_perfiles` SET `tipos_materiales_id`='{$tipos_materiales_id}',`cantidad`='{$cantidad}',`observacion`='{$observacion}',`elemento_diagnostico`='{$elemento_diagnostico}',`observacion_elemento_diagnostico`='{$observacion_elemento_diagnostico}' WHERE `material_estrato_perfil_id`='{$material_estrato_perfil_id}'";
      	//echo $sql;
